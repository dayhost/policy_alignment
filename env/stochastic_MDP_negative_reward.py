...
class StochasticNegativeMDP(object):
    ...
    def _get_transition_probability(self):
        ...
        non_terminal_2_terminal_transition_probability = np.zeros(shape=[non_terminal_state_size, self.terminal_state_size, self.action_size])
        ...
        # construct the only policy that can reach terminal state
        # 构建仅仅只有一个能到terminal state value的策略，在reward function为负的时候，只有这个策略是最优的
        choose_action = np.random.choice(self.action_size, 1)[0]
        choose_non_terminal_state = np.random.choice(non_terminal_state_size, 1)[0]
        for choose_state_j in range(self.terminal_state_size): 
            non_terminal_2_terminal_transition_probability[choose_non_terminal_state][choose_state_j][choose_action] = np.random.uniform(low=1, high=2, size=1)[0]
    ...
    def _get_reward_function(self):
        ...
        non_terminal_reward_func = np.random.uniform(
            low=self.r_max - 0.1, high=self.r_max, size=(non_terminal_state_size, non_terminal_state_size, self.action_size))
        non_terminal_2_terminal_reward_func = np.random.uniform(
            low=self.r_min, high=self.r_min + 0.1, size=[non_terminal_state_size, self.terminal_state_size, self.action_size])
